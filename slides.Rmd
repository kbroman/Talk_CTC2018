...
```{r geno_freq_bymar}
geno_freq_bymar <- function(density=FALSE) {
...
```{r geno_freq_bymar_wdensity}
geno_freq_bymar(TRUE)
# odd markers:
...
```{r genotype_errors_bymar, dev="svg", doi=600}
...
par(mar=c(5.1,12.1,2.1,12.1), cex=0.9, cex.axis=2.3, cex.lab=2.3)
grayplot(prop_missing_bymar, percent_err_bymar,
         xlim=c(0, 92), ylim=c(0, 72), xaxs="i", yaxs="i",
         xlab="Percent missing data", ylab="Percent errors")
# ICR107 (70% error rate)
# UNC5698022, ICR5650 (50% error rate)
# UNC5698022 looks perfectly clean; my guess is it's misplaced
# ...placed on chr 11, but map it as a phenotype and it maps to chr 3
# UNCJPD000988 on chr 2 but maps to chr 6
# UNC3640234 on chr 18 but maps to chr 2
# out <- scan1(apr_fst, g[,mar<-"UNC3640234",drop=FALSE], cores=0)
# plot(out, gmap);find_markerpos(gmap, mar)
# UNCJPD001874 on chr 4 but maps to chr 5
# UNCJPD002501 on chr 6 but maps to chr 8 (allele intensity plot still bad)
# UNC17113030 on chr 14 but maps to chr 9 (allele intensity plot is clean)
#
# UNCHS021230 is interesting: reasonably clean allele intensity plot,
# but most clusters deemed to be errors; want to color them by genotype
# determined from genotype probabilities
# ...UNCHS027234 looks just like that; also UNCHS006327
#
# UNCrs232949530 is just total crap genotype calls...can kind of see
# what it should be
#
# UNCHS000060 is total total crap SNP allele intensity plot; nothing
# would work for this one
#
# UNCJPD002501, UNCrs50250732, UNCrs241258463 are similarly bad
#
# Mit0045 has two clear clusters but errors everywhere
#
# UNCHS028710 looks really terrible but in an interesting way
```
